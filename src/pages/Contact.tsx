...
export const Contact: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (endpoint) {
            ...
            // No backend configured: simulate success for dev/demo
            setTimeout(() => {
                setFormStatus('success');
                setFirstName('');
                setLastName('');
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      <PageMeta title="Contact" description="Get in touch with HerRise Development Organisation. We welcome questions, suggestions, and partnership inquiries." />
            ...
            <div className="bg-white p-8 rounded-xl shadow-lg h-fit">
                ...
                {formStatus === 'success' ? (
                    ...
                    <div className="bg-red-50 text-red-800 p-6 rounded-lg text-center">
                        ...
                        <p>Your message could not be sent. Please try again or email us directly.</p>
                        <button 
                            onClick={() => setFormStatus('idle')}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" required value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                                <input type="text" required value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
                            <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Subject</label>
                            <select value={subject} onChange={(e) => setSubject(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none">
                        ...
                        <div>
                            ...
                            <textarea required rows={4} value={message} onChange={(e) => setMessage(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none"></textarea>
                        </div>
                        <button 
                            type="submit" 
